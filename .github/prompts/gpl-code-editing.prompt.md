...
## 기본 원칙
- 이 저장소는 단발 기능 모음이 아니라 **제대로 된 재사용 가능한 PA GPL 라이브러리의 기반 틀**을 만드는 공간으로 본다.
- 변경은 국소 편의보다 **호환성**, **확정성**, **유연성**, **재사용성**을 우선한다.
- 기존 호출부, 데이터 형식, 제어기 동작을 가능하면 깨지 않도록 유지한다.
- 호환성·컴파일 위험·호출부 변경은 *해결할 비용*으로 다루고, 옳은 구조를 타협하는 핑계로 쓰지 않는다. 단 파괴적·대규모 변경은 사용자 승인 후 진행한다.
...
## 구조와 파일/모듈 분할 기준
- 파일/모듈 분할·병합은 **재사용·교체 경계(응집도/결합도)** 기준으로 판단한다. **줄 수(파일 길이)는 그 자체로 분할 사유가 아니다.**
- 길이가 길어도 단일 도메인이면 한 파일에 두는 것이 옳다. 예: `Format_Apon`(APN)의 `AponNode → AponFile → AponStore`는 한 포맷 도메인의 계층 스택이라 1파일 다중 클래스가 정답이며, `AponStore`는 영속화를 `PIP`/`PST`에 위임해 책임 분리가 이미 깔끔하다.
- 분할은 독립적으로 재사용·교체·테스트될 **경계가 실제로 출현할 때** 한다. 예: `Networking_TcpServer`(NET)는 전송(소켓/세션/스레드)과 프로토콜(`EvaluateLine`/`TryHandleCustom`) 이음새가 있어, 명령 셋이 커지면 분리 후보가 된다.
- GPL 사실: 한 `.gpl` 파일은 여러 Module/Class를 담을 수 있다(공식). "파일 1 : Module 1"은 이 프로젝트의 관례일 뿐 GPL 강제 규칙이 아니다. 단 같은 Module명을 여러 파일로 분산하는 방식은 공식 미문서화(미지원 가능성)이므로, 그 방식을 전제로 한 계획은 먼저 검증한다.
- 분할 시에는 가능한 한 기존 한정 참조(`APN.`, `NET.` 등)를 보존하는 방식을 우선 검토한다.
